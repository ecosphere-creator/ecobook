...
//! -- most importantly from `auth_client.rs`/`payments_client.rs`/
//! `community_client.rs`'s peer HTTP calls, so it can be forwarded
//! downstream without threading it through every handler signature.
use axum::{
    extract::Request,
